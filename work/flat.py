...
#
# **選択なのだから、測ら できなければならない。** 環境で下げれば
# 「小さい空間も家として渡す」を試せる（1 にすれば全部が家になる）。
# いまの既定 512 では、コーパス 2,616 本の規則のうち家になるのは 12 本である
# （`work/count_family.py`）—— 選択が既定を作っているのであって、
# 多面体の道が狭いのではない。
FAMILY_MIN = int(os.environ.get('LATTIX_FAMILY_MIN', '512'))
