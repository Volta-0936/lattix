...
TEMPLATES = [
    ...
    # 構成子の引数の塊の頭（場所の記録）: 穴 0 = 出現の名のトークン、穴 2 = 引数の番号
    (116, "\n_sv[_st{0:6}[_r{1:1}"), (117, "\n_sv[_st{0:6}[%"), (118, "\n_sv[_st{0:6}[_z"),
    (119, "\n_sva[_st{0:6}[_r{1:1}"), (120, "\n_sva[_st{0:6}[%"), (121, "\n_sva[_st{0:6}[_z"),
    (122, "], {2:1}] <- "), (123, ", _r{1:1}], {2:1}] <- "), (124, ", %], {2:1}] <- "),
    # 構成子の出現の本文の参照 `_sid[_st<A>[鍵]]`
    (125, "_sid[_st{0:6}[_r{1:1}"), (126, "_sid[_st{0:6}[%"), (127, "_sid[_st{0:6}[_z"),
    (128, "]]"), (129, ", _r{1:1}]]"), (130, ", %]]"),
    # 出現の帯
...
def compress(text, limit=90):
    ...
    置き換える。句の字は型紙の帯に 160 + m 番の型紙として入れる —— 開くのは型紙を開く規則そのもので、
    句のバイト p は `ex[p, k] = tpc[p のバイト + 32, k]`（⊥ は幅 0 なので、句の長さを数えずに並ぶ）。
