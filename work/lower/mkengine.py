...
ROWS = [("_o", "max", "O"), ("_pv", "max", 0), ("_pk1", "max", 0), ("_pk2", "max", 0), ("_pp", "max", 0),
        ...
        # **構成子の無い本では使わない**（"V": 構成子が在れば行 × `_o` の広さ、無ければ 1 × 1）。
        # 十二枚が行 × 116 升ずつ場を取り、print の大きい本（25_eval・32_shape・33_self）を
        # 焼き手の置き場（2 GB）から溢れさせていた（理由 E）—— 書かれない場の置き場だった。
        ("_V0", "flat", "V"), ("_V1", "flat", "V"), ("_V2", "flat", "V"), ("_V3", "flat", "V"), ("_V4", "flat", "V"),
        ("_V5", "flat", "V"), ("_V6", "flat", "V"), ("_V7", "flat", "V"), ("_V8", "flat", "V"),
        ("_VN", "flat", "V"), ("_VD", "flat", "V"), ("_VG", "flat", "V"),
...
field _sad : flat bound 16384                # 場所 g の番地（出現の塊が写す —— アクセサが比べて引く）
