...
# **座標は入れ子になれる** `o[p[i,j]]` —— 内側の読みが **二次元**。
#   源2/3 は内側の次元を一本しか持てなかったので、内側の次元1 が黙って落ち、
#   別の升を読んでいた（o[0] と o[1] にしか書かない）。内側を一つの所有者に
#   すると、次数にも入れ子の深さにも上限が要らない。
SRC13B = ("table ch = (0,32)\n"
          "field p : max bound 4 4\n"
          "p[0,0] <- 0\np[0,1] <- 1\np[1,0] <- 2\np[1,1] <- 3\n"
          "field o : max bound 8\n"
          "o[p[i,j]] <- 5   for (i) in 0 .. 1 for (j) in 0 .. 1\n")
...
#          所有者種 0 項 / 1 ガード / 2 書き先 / **3 内側の読み**
#          源 0 列 / 1 計数器 / 2,3 場の値 / 4 定数 / **5 内側の所有者が作る座標**
...
cases = [
    ...
    # **座標は入れ子になれる**（内側の読みが二次元。所有者が所有者を持つ）
    ("入れ子の座標 o[p[i,j]]", SRC13B, ['p', 'o'],
     [(0, 2, 2, 4, 4), (1, 2, 1, 8, 8)],
     [(0, 0, 0), (0, 1, 1), (0, 4, 2), (0, 5, 3)],
     [(0, 2, 0, 1)], [], [],
     [(0, 1, 0, 0, 5)],
     [C(3, 0, 0, 0, 1, 0, 0),                       # 内側 次元0: 計数器 0（i）
      C(3, 0, 0, 1, 1, 0, 1),                       # 内側 次元1: 計数器 1（j）
      C(2, 0, 0, 0, 5, 0, 0)],                      # 書き先: 源5（内側の所有者 0）
