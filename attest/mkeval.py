...
a(f"""
# ══ 種（寄与は階数 0 の読みから）═══════════════════════════════════════════
# 種は 16,384 まで（焼き手の種の箱と同じ。前は 4,096 —— 14w で accept に種 8,200 の本を足した日、ir は ATTESTED、
# attest.lx は種の箱 aS を越えて終了コード 6 で止まり、そう名で言った）
field aS : max bound 16384
...
field scel : max bound 16384
scel[s] <- fcb[sdf[s]] + sdc[s]   for (s) in 0 .. nsdz[0]
field slt : max bound 16384
slt[s] <- flt[sdf[s]]   for (s) in 0 .. nsdz[0]
field sval : flat bound 16384                    # 種の値の四倍（count は 1、or は 1）
